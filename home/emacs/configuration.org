...
bound to C-c C-v t). This function is also run automatically when this
buffer is saved.

* Current state of this configuration

- right now I'm not really using emacs for my daily needs.
  - why?
    - I'm really used to the "select -> action" flow of helix.
    - I have a terminal centric interaction with my computer
      - navigate to a project using cd
      - direnv automatically puts me in an environment where I have
        any dependency I would need.
    - I want to use my window manager to manage my windows, not emacs.
      - this can be solved by altering how emacs creates new windows
        to instead create frames in some cases
    - I have many times tried to get a comfy config going but always
      burn out before I actually start using the config.
    - It could also be that I need to adapt myself to the 'emacs way'.
  - why do I still feel that I would even want to use emacs?
    - the idea of investing in timeless tools.
    - though It could be that most of my needs will be met when
      plugins land in helix.
      
- open questions
  - to aid in discovering my bindings, I need to fully understand how
    the emacs keymap system works, and how I should interact with it
    through meow, use-package and which-key.
  - Right now I'm using eza aliased to cd to navigate around my
    computer.
    - For some reason I find it a bit 'clastrophobic' to do all my
      interaction with my computer through emacs.
...
;; -*- lexical-binding: t -*-
;; This file has been generated from the configuration.org file. DO NOT EDIT.
;; Sources are available from https://github.com/lakrestofer/dotfiles/tree/main/.config/emacs
#+end_src

#+begin_src emacs-lisp :tangle early-init.el  
;; -*- lexical-binding: t -*-
;; This file has been generated from the configurations.org file. DO NOT EDIT.
;; Sources are available from https://github.com/lakrestofer/dotfiles/tree/main/.config/emacs
#+end_src
** Early init
...
Here we install elpaca and setup its integration with use-package
#+begin_src emacs-lisp :tangle init.el
...
To aid in keybind discovery we install which-key. This adds a
interactive popup that becomes visible after a short delay.
