...
function normalizeTodoSettings(raw: Partial<TodoSettings>): TodoSettings {
	...
	const gcDays = typeof raw.gcDays === "number" ? raw.gcDays : DEFAULT_TODO_SETTINGS.gcDays;
	return {
		gc: Boolean(gc),
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			const singleDetails = details as { action: "get" | "create" | "update" | "append" | "delete" | "claim" | "release"; todo: TodoRecord; error?: string };
			...
			const actionLabel =
				singleDetails.action === "create"
					? "Created"
					: singleDetails.action === "update"
						? "Updated"
						: singleDetails.action === "append"
							? "Appended to"
							: singleDetails.action === "delete"
								? "Deleted"
								: singleDetails.action === "claim"
									? "Claimed"
									: singleDetails.action === "release"
										? "Released"
										: null;
			if (actionLabel) {
